...
public class ImageGLSurfaceView extends GLSurfaceView implements Renderer {
    ...
    public void setFilterWithConfig(final String config) {
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                if (mImageHandler == null) {
                    Log.e(LOG_TAG, "set config after release!!______"+ config );
                    return;
                }
...
//        setZOrderMediaOverlay(true);
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        mViewHeight = height;
        calcViewport();
        mBitmap = createBitmapFromGLSurface(gl);
    }
...
//    }
    ...
    public Bitmap createBitmapFromGLSurface( GL10 gl)
            throws OutOfMemoryError {
        int bitmapBuffer[] = new int[mRenderViewport.width * mRenderViewport.height];
        int bitmapSource[] = new int[mRenderViewport.width * mRenderViewport.height];
        IntBuffer intBuffer = IntBuffer.wrap(bitmapBuffer);
        intBuffer.position(0);

        try {
            gl.glReadPixels(mRenderViewport.x, mRenderViewport.y, mRenderViewport.width, mRenderViewport.height, GL10.GL_RGBA, GL10.GL_UNSIGNED_BYTE, intBuffer);
            int offset1, offset2;
            for (int i = 0; i < mRenderViewport.height; i++) {
                offset1 = i * mRenderViewport.width;
                offset2 = (mRenderViewport.height - i - 1) * mRenderViewport.width;
                for (int j = 0; j < mRenderViewport.width; j++) {
        ...
        return Bitmap.createBitmap(bitmapSource, mRenderViewport.width, mRenderViewport.height, Bitmap.Config.ARGB_8888);
    }
